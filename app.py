...
def index():
    ...
    # Statistik
    top_category =pd.crosstab(index=df2['Category'],columns='Jumlah',values=df2['Category'],aggfunc='count').\
    sort_values(by='Jumlah',ascending = False).reset_index()
    # Dictionary stats digunakan untuk menyimpan beberapa data yang digunakan untuk menampilkan nilai di value box dan tabel
    stats = {
        'most_categories' : top_category.iloc[0,0],
        'total': top_category.iloc[0,1],
        'rev_table' : df2[['Category','App','Reviews','Rating']].groupby(['Category','App']).max().\
    ...
    plt.barh(cat_order.index,cat_order.Total, color=my_colors)
    ...
    A=(df2.Installs/1000).values
    fig = plt.figure(figsize=(5,5))
    fig.add_subplot()
    plt.hist(A,bins=10, density=True,  alpha=0.75)
    plt.xlabel('Installs')
    plt.ylabel('Frequency')
